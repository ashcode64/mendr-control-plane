 ...
 * Row-Level Security. The identifier may already be a tenant UUID (gateway-resolved or
 * the default tenant) or a WorkOS organization id, which is mapped via the {@code tenants}
 * registry (mirrors the gateway's {@code TenantRepository.findByWorkosOrgId}). Results are
 * cached since the org id -> tenant mapping is effectively immutable.
 */
@Slf4j
@Component
