 ...
 * so that {@code TenantContextFilter} may trust it. Runs before authorization so a valid key
 * satisfies {@code authenticated()} when enforcement is on. JWT bearer tokens are left to the
